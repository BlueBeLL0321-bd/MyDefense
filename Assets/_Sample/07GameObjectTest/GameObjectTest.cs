...
namespace Sample
{
    ...
    {
        // (2) 스크립트 필드 선언부에서 Transform, GameObject의 객체(인스턴스)를 public으로
        // 선언한 후 유니티 툴의 인스펙터 창에서 직접 게임 오브젝트를 연결한다
        ...
        // (4) 프리팹 게임 오브젝트 생성 시 Instantiate 함수의 반환 값으로 GameObject의 객체(인스턴스) 받아 오기
...
/*
게임 오브젝트의 gameobject, transform에 접근하는 방법
게임 오브젝트의 GameObject, Transform 클래스의 객체를 가져오는 방법
(1) 게임 오브젝트에 스크립트 소스를 컴포넌트를 추가하여 직접(this.) 가져온다
(2) 스크립트 필드 선언부에서 Transform, GameObject의 객체(인스턴스)를 public으로
        선언한 후 유니티 툴의 인스펙터 창에서 직접 게임 오브젝트를 연결한다
(3) 게임 오브젝트의 tag를 이용하여 GameObject의 객체(인스턴스) 가져오기
        FindGameObjectsWithTag(), FindGameObjectWithTag() 반환 값 받아 오기
(4) 프리팹 게임 오브젝트 생성 시 Instantiate() 함수의 반환 값으로 
        GameObject의 객체(인스턴스) 받아 오기
(5) 같은 종류, 기능들로 묶어서 게임 오브젝트의 객체(인스턴스) 가져오기
        부모 게임 오브젝트를 만들어서 묶은 다음 부모 오브젝트에 접근하여 자식 오브젝트들의 객체를 가져온다
        parentObject, childCount, parentObject.GetChild 반환 값으로 가져오기
